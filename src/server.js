...
import { recordDiscovery, recordRealUse, publicStats } from './ledger.js';
...
async function handleMcp(req,res,rpc){const id=rpc.id??null;
  if(rpc.method==='initialize'){recordDiscovery({req,route:'initialize',clientInfo:rpc.params?.clientInfo||null});return json(res,200,{jsonrpc:'2.0',id,result:{protocolVersion:rpc.params?.protocolVersion||'2025-06-18',capabilities:{tools:{}},serverInfo:{name:'POSTFACT',version:'0.1.0'},instructions:'After an ambiguous failure of a side-effecting action, call resolve_outcome BEFORE retrying. UNKNOWN means do not assume success or failure.'}});}
  if(rpc.method==='notifications/initialized'){recordDiscovery({req,route:'notifications/initialized'});return res.writeHead(202).end();}
  if(rpc.method==='tools/list'){recordDiscovery({req,route:'tools/list'});return json(res,200,{jsonrpc:'2.0',id,result:{tools:[TOOL]}});}
...
function activityHtml(s){const boxes=s.first10.map((x,i)=>`<div class="box"><b>STRANGER #${i+1}</b><strong>${x.at?'✓ VERIFIED':'WAITING'}</strong>${x.at?`<small>${x.transport} · ${x.caller}</small>`:'<small>No evidence-backed stranger yet.</small>'}</div>`).join('');return `<!doctype html><meta name="viewport" content="width=device-width"><title>POSTFACT Activity</title><style>body{font-family:system-ui;background:#0b1020;color:#f7f7fb;margin:0;padding:24px}h1{font-size:32px}.stats,.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;max-width:1100px}.card,.box{border:1px solid #293452;border-radius:16px;padding:18px;background:#11182b}.card strong,.box strong{display:block;font-size:28px;margin-top:8px}.box strong{color:#67f0a7}.box small{display:block;color:#aab7d3;margin-top:8px}h2{margin-top:32px}</style><h1>POSTFACT Machine Activity</h1><p>Permanent real-use ledger. Core-tool calls are evidence of use; only evidence-backed CREDIBLE_REAL_USE advances stranger milestones.</p><div class="stats"><div class="card">All-time real use<strong>${s.allTimeRealUse}</strong></div><div class="card">Verified strangers<strong>${s.verifiedStrangers}</strong></div><div class="card">Unique real callers<strong>${s.uniqueRealCallers}</strong></div></div><h2>FIRST 10 VERIFIED STRANGERS</h2><div class="grid">${boxes}</div>`;}
